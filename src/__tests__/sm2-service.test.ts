...
describe("SM2Service", () => {
  ...
  beforeAll(async () => {
    ...
    if (!isJavaAvailable) {
      throw new Error(
        "Java is not available. Tests require Java 17+ to be installed."
      );
    }
    ...
    if (!isServiceAvailable) {
      throw new Error(
        "SM2 service is not available. Please build the Java component: npm run build:java"
      );
    }
  ...
  describe("Decryption", () => {
    ...
    it("should handle multiple encrypt-decrypt cycles", async () => {
      const plaintexts = ["First message", "Second message", "Third message"];
  ...
  describe("Encryption and Decryption Integration", () => {
    ...
    it("should work with newly generated key pairs", async () => {
      ...
      const newService = new SM2Service(
        newKeyPair.privateKey,
        newKeyPair.publicKey
      );

      const plaintext = "Test with new keys";
      const encrypted = await newService.encrypt(
        plaintext,
        newKeyPair.publicKey
      );
    ...
    it("should handle newline characters", async () => {
      ...
      const encrypted = await sm2Service.encrypt(
        textWithNewlines,
        testPublicKey
      );
      const decrypted = await sm2Service.decrypt(encrypted);
  ...
  describe("Error Handling", () => {
    it("should throw error when encrypting without public key", async () => {
      ...
      await expect(serviceWithoutPublicKey.encrypt("test")).rejects.toThrow(
    ...
    it("should throw error when decrypting empty string", async () => {
      await expect(sm2Service.decrypt("")).rejects.toThrow();
    });
